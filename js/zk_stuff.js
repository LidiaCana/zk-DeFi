async function getMerklePath(leaves) {
  ...
  while (layers[layers.length - 1].length > 1) {
    ...
    for (let i = 0; i < currentLayer.length; i += 2) {
      ...
      nextLayer.push(
        await poseidonContract.methods.poseidon([left, right]).call()
      );
    }
...
async function getWithdrawalProof(
  index,
  privateKey,
  nullifier,
  recipient,
  ...
  const { proof, publicSignals } = await snarkjs.groth16.fullProve(
    proverParams,
    "../zk_artifacts/proveWithdrawal.wasm",
    "../zk_artifacts/proveWithdrawal_final.zkey"
  );
  ...
  document.getElementById("web3_message").textContent =
    "Proof generated please confirm transaction.";
