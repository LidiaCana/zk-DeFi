...
const HURACAN_ADDRESS = "0xc010553F6Cf99E73E147f79c63850598511A4aD2";
    ...
    web3.eth.net.getId((err, netId) => {
      ...
      if (netId == NETWORK_ID) {
        var awaitContract = async function () {
          huracanContract = await getContract(
            web3,
            HURACAN_ADDRESS,
            HURACAN_ABI_PATH
          );
          poseidonContract = await getContract(
            web3,
            POSEIDON_ADDRESS,
            POSEIDON_ABI_PATH
          );
...
const deposit = async (depositPrivateKey, depositNullifier) => {
  let commitment = await poseidonContract.methods
    .poseidon([depositPrivateKey, depositNullifier])
    .call();
  ...
  const result = await huracanContract.methods
    .deposit(commitment)
...
const withdraw = async (privateKey, nullifier, recipient) => {
  ...
  let proof = await getWithdrawalProof(
    index,
    privateKey,
    nullifier,
    recipient,
